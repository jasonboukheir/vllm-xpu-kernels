...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16],
                         ids=format_tc)
@torch.inference_mode()
def test_qwen38_gdn_chunk_scratch_survives_allocator_pressure(dtype):
    """Keep the asynchronous two-chunk A/w/u workspaces alive.

    The fused Xe2 path allocates these workspaces inside the C++ wrapper and
    returns after enqueueing their final consumer. Reallocating the exact
    shapes immediately after the call catches a missing allocator stream
    record without loading the full model.
    """
    device = "xpu"
    random.seed(0)
    torch.manual_seed(0)
    ...
    padding_elems = 4096
    state_elems = cache_batch_size * num_v_heads * head_v_dim * head_k_dim
    state_dtype = torch.float32 if dtype == torch.bfloat16 else dtype
    state_backing = torch.full(
        (state_elems + 2 * padding_elems,),
        113.0,
        dtype=state_dtype,
        device=device,
    )
    ssm_state = state_backing[padding_elems:-padding_elems].view(
        cache_batch_size, num_v_heads, head_v_dim, head_k_dim)
    ssm_state.normal_()
    ref_ssm_state = ssm_state.clone()
    ...
    # Retain q/k/v/b/a in Python so the pressure below isolates the three
    # function-local delta-rule workspaces rather than the fused wrapper's
    # outer intermediate lifetime.
    intermediates = torch.ops._xpu_C.causal_conv1d_non_spec(
        z,
        projected_states_qkvz,
        projected_states_ba,
    ...
    # Clear unrelated free blocks, then enqueue the production BF16/FP32-state
    # delta route on a non-default stream. Without allocator stream records,
    # the default-stream churn can immediately recycle its local A/w/u.
    ...
    # Match the internal size classes before synchronizing. Keeping every
    # replacement live prevents the caching allocator from hiding a premature
    # release by cycling through the same block again.
    ...
    torch.testing.assert_close(
        conv_state, ref_conv_state, atol=atol, rtol=rtol)
    torch.testing.assert_close(
        core_attn_out, ref_core_attn_out, atol=atol, rtol=rtol)
    torch.testing.assert_close(
        ssm_state, ref_ssm_state, atol=atol, rtol=rtol)
    torch.testing.assert_close(
    ...
    torch.testing.assert_close(
        state_backing[:padding_elems],
        torch.full_like(state_backing[:padding_elems], 113.0),
        rtol=0,
        atol=0,
    )
    torch.testing.assert_close(
