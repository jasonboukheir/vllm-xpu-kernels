...
def _assert_bitwise_equal(actual: torch.Tensor, expected: torch.Tensor,
    ...
    if torch.equal(actual_cpu.view(torch.int16), expected.view(torch.int16)):
        return
    different = actual_cpu.view(torch.int16) != expected.view(torch.int16)
...
def _require_native_attention(monkeypatch: pytest.MonkeyPatch) -> None:
    def reject_fallback(*args, **kwargs):
        pytest.fail("replay probe must execute the native attention kernel")

    monkeypatch.setattr(flash_attn_interface, "_fallback_varlen_attn",
                        reject_fallback)


def _assert_reference(actual: torch.Tensor, q: torch.Tensor,
                      k: torch.Tensor, v: torch.Tensor,
                      kv_len: int, block_table: torch.Tensor | None = None):
    # CPU FP32 reference avoids sharing native attention arithmetic.
    if block_table is None:
        block_table = torch.empty((1, 0), dtype=torch.int32)
    reference = ref_paged_attn(
        q.cpu().float(), k.cpu().float(), v.cpu().float(),
        query_lens=[q.shape[0]], kv_lens=[kv_len],
        block_tables=block_table.cpu(), scale=_HEAD_DIM**-0.5,
        window_size_left=-1, window_size_right=-1,
        is_paged=k.ndim == 4, casual=True, dtype=torch.float32)
...
def test_frozen_shape_dense_causal_prefill_is_bitwise_deterministic(
    ...
    _assert_reference(expected, *target, seq_len)
...
def test_frozen_shape_paged_causal_prefill_is_bitwise_deterministic(
    ...
    _assert_reference(expected, target_q, *target_cache[:2], seq_len,
                      target_cache[2])
    for repeat in range(_REPEATS):
...
def test_frozen_shape_paged_decode_is_bitwise_deterministic(
    ...
    _assert_reference(expected, target_q, *target_cache[:2], kv_len,
                      target_cache[2])
    for repeat in range(_REPEATS):
