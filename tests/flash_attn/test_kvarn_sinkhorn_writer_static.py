...
XE2_ATTN_CMAKE = (REPO_ROOT / "csrc/xpu/attn/xe_2/CMakeLists.txt").read_text()
WRITER = (
...
def test_record_writers_inherit_global_o3_policy() -> None:
    assert '"-O3" "-DNDEBUG"' in ROOT_CMAKE
    assert "-fp-model=precise" not in ROOT_CMAKE
    assert "-fp-model=precise" not in XE2_ATTN_CMAKE
    assert "KVARN_PRECISE_FP_SOURCES" not in XE2_ATTN_CMAKE
    assert "set_source_files_properties" not in XE2_ATTN_CMAKE
