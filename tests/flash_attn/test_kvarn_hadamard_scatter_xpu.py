...
def test_kvarn_hadamard_query_scatter_accepts_fp16_query_bf16_kv():
    """Match the castless model contract used by native KVarN decode."""
    _load_op()
    generator = torch.Generator().manual_seed(20260810)
    query = torch.randn(24, 256, generator=generator).half().xpu()
    key = torch.randn(1, 4, 256, generator=generator).bfloat16().xpu()
    value = torch.randn(1, 4, 256, generator=generator).bfloat16().xpu()
    slots = torch.tensor([129], dtype=torch.int64, device="xpu")
    lookup = torch.tensor([-1, 0], dtype=torch.int32, device="xpu")
    ...
    torch.testing.assert_close(
        query_out, torch.matmul(query, h), atol=2e-3, rtol=2e-3
    )
    torch.testing.assert_close(
        tail_key[0, 1], torch.matmul(key.half(), h)[0], atol=2e-3, rtol=2e-3
    )
    torch.testing.assert_close(
        tail_value[0, 1],
        torch.matmul(value.half(), h)[0],
        atol=2e-3,
        rtol=2e-3,
    )
